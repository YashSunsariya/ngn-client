...
const Register = () => {
  ...
  const formik = useFormik({
    initialValues: { name: '', email: '', contact: '', address: '', password: '' },
    validate: (values) => {
      const errors = {}
  ...
  return (
    <Box sx={{ minHeight: '100vh', display: 'flex' }}>
      ...
      >
        ...
        <Box sx={{ position: 'relative', textAlign: 'center', px: 4 }}>
          ...
          <Typography sx={{ ...DS.typography.body, color: 'rgba(255,255,255,0.8)', maxWidth: 320, mx: 'auto', lineHeight: 1.7 }}>
            Create your account to get started with our services and solutions.
          </Typography>
        </Box>
      </Box>
      ...
      >
        <Box sx={{ width: '100%', maxWidth: 420 }}>
          ...
          <Box component="form" onSubmit={formik.handleSubmit}>
            <Stack spacing={2.5}>
              <TextField label="Full name" name="name" placeholder="John Doe" fullWidth autoComplete="name" autoFocus
                value={formik.values.name} onChange={formik.handleChange} onBlur={formik.handleBlur}
                error={formik.touched.name && Boolean(formik.errors.name)} helperText={formik.touched.name && formik.errors.name}
                sx={DS.input}
              />
              <TextField label="Email address" name="email" type="email" placeholder="you@example.com" fullWidth autoComplete="email"
                ...
                sx={DS.input}
              />
              <TextField label="Contact number" name="contact" placeholder="1234567890" fullWidth autoComplete="tel"
                value={formik.values.contact} onChange={formik.handleChange} onBlur={formik.handleBlur}
                error={formik.touched.contact && Boolean(formik.errors.contact)} helperText={formik.touched.contact && formik.errors.contact}
                sx={DS.input}
              />
              <TextField label="Address" name="address" placeholder="Street, City, State" fullWidth autoComplete="street-address"
              ...
              />
              <TextField
                label="Password" name="password" type={showPassword ? 'text' : 'password'} fullWidth autoComplete="new-password"
                ...
                InputProps={{
                  endAdornment: (
                    <InputAdornment position="end">
                      <IconButton aria-label="toggle password visibility" onClick={() => setShowPassword((p) => !p)} edge="end" size="small">
          ...
          <Typography sx={{ ...DS.typography.body, mt: 3, textAlign: 'center', color: DS.colors.sub }}>
            ...
            <Link component={RouterLink} to="/login" sx={{ color: DS.colors.primary, fontWeight: 600, textDecoration: 'none', '&:hover': { textDecoration: 'underline' } }}>
              Sign in
            </Link>
          </Typography>
        </Box>
      ...
      >
        <Alert severity={snackbar.severity} variant="filled" sx={{ borderRadius: DS.radii.md, fontFamily: DS.fonts.body }}>
          {snackbar.message}
        </Alert>
      </Snackbar>
    </Box>
